...
--as5446
--ms5526
--edited to give only the output needed to generate output.txt
require 'torch'
require 'nn'
require 'nngraph'
-- exotics
    ...
    for k=1,#sents do
      ...
      if opt.dump_images == 1 then
        ...
        local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" vis/imgs/img' .. #predictions .. '.jpg' -- bit gross
       	--file = io.open('/home/shreya94jain/torch/neuraltalk2/results.txt')
	print(data.infos[k].file_path,'\t',entry.caption)
	--file:close()
	print(cmd)
...
if opt.dump_json == 1 then
  -- dump the json
  --s = string.format('image %s: %s', entry.image_id, entry.caption)
 -- utils.write_json('vis/vis.json', s)
